...
const nav = [
  ...
  // { href: "/admin/chat", label: "Chat", icon: MessageSquare }, // Placeholder if we add chat later
];
    ...
    <div className="flex min-h-screen bg-brand-950 text-white">
      ...
      >
        <div className="h-16 px-4 flex items-center justify-between border-b border-brand-800">
          <span className={`font-bold text-lg text-accent-blue truncate ${collapsed ? "opacity-0 w-0 overflow-hidden" : "w-auto"}`}>
            Admin
          </span>
          <button
        ...
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          {nav.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={`h-5 w-5 shrink-0 ${active ? "text-accent-blue" : "text-brand-500 group-hover:text-white"}`} />
                <span className={`transition-opacity duration-200 ${collapsed ? "opacity-0 w-0 overflow-hidden" : "opacity-100 w-auto"}`}>
                  {item.label}
                </span>
              </Link>
            );
