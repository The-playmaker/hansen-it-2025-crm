...
/**
 * Resolves a quote by quotes.id first.
 *
 * Fallback via quotes.source_request_id exists ONLY for backward compatibility with
 * old customer-facing links that still carry a request id. It is not part of the
 * current data model — do not rely on it for new code paths.
 */
...
export async function resolveQuoteId(inputId) {
  ...
  if (viaRequest.data) {
    console.warn(
      "[resolveQuoteId] source_request_id fallback used (legacy link). Prefer quotes.id going forward.",
      { inputId: id, quoteId: viaRequest.data.id }
    );
