...
function mainSummary(report, actions) {
  ...
  return [
    ...
    `Vår vurdering er: ${priorityLabel(report.score)}. Det viktigste nå er å starte med ${String(top).toLowerCase()}.`,
    "Vi anbefaler å starte med HTTPS/TLS og e-postbeskyttelse der dette er relevant. Dette er normalt overkommelig arbeid og gir høy sikkerhetsgevinst.",
...
export async function downloadSecurityReportPdf(input, options = {}) {
  ...
  for (const line of mainSummary(report, mainActions)) {
  ...
  card(margin + 56, y, 48, 30, "Vurdering", priorityLabel(report.score), severityColor(mainActions[0] ? severityOf(mainActions[0]) : "low"), 10);
  ...
  infoBox(
    "Første tiltakspakke: ca. 3-6 timer",
    "Start med de funnene som påvirker tillit, HTTPS/TLS og e-postbeskyttelse. Full gjennomgang og alle mindre forbedringer avtales etter behov.",
    COLORS.amber,
    30
  );
  ...
  infoBox(
    "Ønsker du at vi fikser dette?",
    "Vi kan lage en konkret tiltakspakke basert på funnene. Dette kan gjøres som fjernarbeid eller avtalt gjennomgang. Fix with Hansen IT betyr kontrollert oppgave- og tilbudsflyt, ikke automatisk retting.",
    COLORS.blue,
    34
  );
