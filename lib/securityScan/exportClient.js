...
export async function downloadSecurityReportPdf(input) {
  const report = safeReport(input);
  const doc = new jsPDF();
  const margin = 18;
  const width = 174;
  ...
  if (logo) {
    doc.addImage(logo, "PNG", margin, y, 66, 19);
    y += 28;
  } else {
    doc.setFontSize(14);
    doc.setFont(undefined, "bold");
    doc.text("Hansen IT", margin, y);
    y += 10;
  }

  doc.setDrawColor(29, 111, 224);
  ...
  y += 7;
  if (report.scannedAt) {
    doc.text(`Dato: ${new Date(report.scannedAt).toLocaleString("nb-NO")}`, margin, y);
    y += 7;
  }
