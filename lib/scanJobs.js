/**
 * Passive job orchestration for Phoenix Scan (CRM / API path).
 *
 * Naming: this is NOT the scanner-node process.
 * - lib/scanJobs.js — claim/run scan_jobs from Next.js (this file)
 * - scripts/scanner-runner.mjs — standalone node on scan01
 *
 * Both paths must call buildSecurityReport() from lib/securityScan/score.js
 * so scores and finding IDs stay identical.
 */
...
import { assessDomainDns, InvalidDomainError } from "@/lib/scanAuthorizationValidation";
...
async function passiveScanDomain(domain, { authorizationSigned = false } = {}) {
  ...
  // Exposed-backend / RLS: only with signed authorization + SCANNER_ALLOW_ACTIVE_SCAN.
  // Still non-destructive (public JS + read-only limit=1 probes).
...
export async function runPassiveScanJob(jobId = null) {
  ...
  if (!domains.length) {
    return { ok: false, status: 400, error: "Passive scan krever minst ett domene i scope." };
  }
